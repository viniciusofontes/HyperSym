...
% -------------------------------------------------------------------------
function PlotMesh_Cylinder(FileName,Nodes,Elem,Supp,Load)
% Plot mesh in Lagrangian (undeformed) configuration.

figure; grid on; axis off; axis tight; axis equal; hold on; view([0 45])
...
if ~isempty(Load)
    h2 = plot3(Nodes(Load(:,1),1),Nodes(Load(:,1),2),Nodes(Load(:,1),3),'g>',...
        'MarkerSize',5,...
        'MarkerFaceColor','green');
    NonzeroBdCnd = 'Pressure';
end

legend([h1,h2],Dofs,NonzeroBdCnd,'Location','ne')
