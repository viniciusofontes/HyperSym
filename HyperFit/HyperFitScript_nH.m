...
Test(3) = struct(...
    'name','pureshear',...
    'data',ps_exp);
%% Define the material model: neo-Hookean
syms A10 lambda1 lambda2 lambda3
I1 = lambda1^2 + lambda2^2 + lambda3^2;
W = A10*(I1 - 3);
PROP = A10; % Array of properties
...
if ismember(portion,{'N','n'})
    splice = @(x,val) x(x(:,1) <= val,:); xval = 1.5;
...
if ismember(portion,{'Y','y'})
    ...
    % Adjust plot with all tests to display only for 1 <= lambda <= 1.5
    for i = 8:-2:2
        ax = figure(1).Children(i); ax.XLim = [1 1.5];
    end

    % Extend the dashed line in all tests (rel. error) plot to lambda = 1
    ax = figure(2).Children(end); ax.XLim = [1 1.5];
